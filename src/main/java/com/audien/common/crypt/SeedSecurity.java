...
public class SeedSecurity implements Serializable  {
	 ...
	 private static byte[] mBase64EncMap, mBase64DecMap; 
	    ...
	    public static byte[] decodeByte(String aData) { 
	        if ((aData == null) || (aData.length() == 0)) 
	...
	public static void SeedDecrypt(byte pbData[], int pdwRoundKey[], byte outData[]) {
		...
		if (!ENDIAN)  {
			EndianChange(L0);  EndianChange(L1);  EndianChange(R0);  EndianChange(R1);
		};
	...
	//���ؾ��ϴ� ����Ű
    public String authKey =   "NdE1e!Tuc#2nlAIv"; //
	...
	public SeedSecurity() {
        byte[] userKey = new byte[nOutSize];
        System.arraycopy(authKey.getBytes(), 0, userKey, 0, authKey.getBytes().length);
        SeedEncRoundKey(pdwRoundKey, userKey);
		
	}
	
	public static String encrypt(String plainStr) {
		
		if("null".equals(plainStr) || plainStr == null){
			return plainStr;
		} else {
			...
			try {
				if(charSet != null) {
					String plainStrCharSet = null;
					plainStrCharSet = new String(plainStr.getBytes(charSet));
					plainStr = plainStrCharSet;
				}
			} catch(UnsupportedEncodingException uee) {
			}
			
			//��ȣȭ ��� ����Ʈ ����
			int iDataLength = plainStr.getBytes().length;
			
			//��ȣȭ ��� ����Ʈ ����
			int iOutSize = (int) (iDataLength / nOutSize) * nOutSize;
			if ((iDataLength % nOutSize) > 0) {
				iOutSize += nOutSize;
			}
			
			//��ȣȭ ��� ����Ʈ
			byte[] outData = new byte[iOutSize];
			//��ȣȭ ��� ����Ʈ
			byte[] pbData = plainStr.getBytes();
			
			System.arraycopy(pbData, 0, outData, 0, iDataLength);
			
			
			
			for (int i = 0; i < iOutSize; i += nOutSize) {
				byte[] DataBlock2 = new byte[nOutSize];
				byte[] DataBlock3 = new byte[nOutSize];
				System.arraycopy(outData, i, DataBlock2, 0, nOutSize);
				
				SeedEncrypt(DataBlock2, pdwRoundKey, DataBlock3);
				
				System.arraycopy(DataBlock3, 0, outData, i, nOutSize);
			}
			
			/*        System.out.println("iOutSize["+iOutSize+"]");
        for (int i = 0; i < iOutSize; i++) {
			System.out.print( Integer.toHexString(0xff&outData[i])+" " );
		}
        ...
        System.out.println("");*/
			...
			return new String(encode(outData)).trim();
		}
	}
	...
	public  static String decrypt(String encStr) {
		...
		if("null".equals(encStr) || encStr == null){
			return encStr;
		}else{
			//��ȣȭ ��� ����Ʈ�� ����
			
			int iDataLength = decodeByte(encStr).length;
			...
			//��ȣȭ ��� ����Ʈ
			byte[] outData = decodeByte(encStr);
			//��ȣȭ ��� ����Ʈ
			byte[] pbData = new byte[iOutSize];
			
			System.arraycopy(outData, 0, pbData, 0, iDataLength);
			

			for (int i = 0; i < iOutSize; i += nOutSize) {
				byte[] DataBlock2 = new byte[nOutSize];
				byte[] DataBlock3 = new byte[nOutSize];
				System.arraycopy(pbData, i, DataBlock2, 0, nOutSize);

				SeedDecrypt(DataBlock2, pdwRoundKey, DataBlock3);

				System.arraycopy(DataBlock3, 0, pbData, i, nOutSize);
			}
			 return new String(pbData).trim();
		}
...
/*	public static void main(String args[]) throws UnsupportedEncodingException{
		String div="E"; // E D
		String authKey="NdE1e!Tuc#2nlAIv";
		String str="1234567890123456";


			SeedSecurity ss = new SeedSecurity(authKey);
			if( "D".equals(div)){
				System.out.print(ss.decrypt(st));	
			}else if("E".equals(div)){
				System.out.print(ss.encrypt(str));	
			}
	}*/
}
